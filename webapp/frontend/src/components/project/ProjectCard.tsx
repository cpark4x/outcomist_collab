...
export function ProjectCard({ project, onDelete, className, viewMode = 'grid' }: ProjectCardProps) {
  ...
  const { tasks, updateTaskFromPhase, completeAllTasks } = useTaskProgress(project.id);
  const { messages, addMessage, updateMessage, refetch: refetchMessages } = useMessages(currentSessionId);

  // Complete all tasks when project status becomes complete
  useEffect(() => {
    if (currentStatus === 'complete') {
      completeAllTasks();
    }
  }, [currentStatus, completeAllTasks]);
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      if (reader) {
          ...
          for (const line of lines) {
            ...
            try {
              ...
              if (parsed.type === 'status_update') {
                ...
                // Update task phase (but not status message - that comes from file creation)
                updateTaskFromPhase(parsed.phase);
                ...
                // Extract file creation activity for task card (this is the detailed status)
                const fileMatch = parsed.content.match(/✓\s+(\w+\.\w+)/);
                if (fileMatch) {
                  updateTaskFromPhase('tool_use', `Creating ${fileMatch[1]}...`);
                }
                ...
                completeAllTasks(); // Mark all tasks as complete
                updateMessage(assistantMessageId, {
                  status: 'complete',
                  progress: {
                    phase: 'complete',
        ...
        // Mark as complete if not already done (keep final progress state)
        updateMessage(assistantMessageId, {
          status: 'complete',
          progress: {
            phase: 'complete',
    ...
    <div className={`bg-[rgba(42,42,42,0.6)] backdrop-blur-[20px] rounded-xl overflow-hidden transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] flex flex-col ${className || 'h-[580px]'} ${
      ...
      >
        ...
        <button
          ...
          className={`p-1 border rounded-md cursor-pointer transition-all duration-200 flex items-center justify-center w-6 h-6 ${
            currentStatus === 'complete' && currentView === 'agent'
              ? 'bg-green-500/20 border-green-500/50 text-green-400 animate-pulse-status hover:bg-green-500/30'
              : 'bg-transparent border-white/10 text-[#888] hover:bg-white/5 hover:text-[#e0e0e0] hover:border-white/20'
          }`}
      ...
      {/* Task Progress Card - Above Input (visible in all views) */}
      <TaskProgressCard tasks={tasks} projectId={project.id} />

      {/* Inline Input (visible in all views) */}
      <InlineInput
